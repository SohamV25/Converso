...
export const getAllCompanions = async ({ limit = 10, page = 1, subject, topic }: GetAllCompanions) => {
    ...
    // Mark which of these companions the current user has bookmarked
    const { userId } = await auth();
    if(!userId || companions.length === 0) {
    ...
    const { data: bookmarks, error: bookmarksError } = await supabase
        ...
        .select('companion_id')
        .eq('user_id', userId)
        .in('companion_id', companions.map((companion) => companion.id));

    if(bookmarksError) throw new Error(bookmarksError.message);

    const bookmarkedIds = new Set(bookmarks.map((bookmark) => bookmark.companion_id));
